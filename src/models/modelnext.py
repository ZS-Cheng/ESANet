...
from src.models.convnext_rgbd import Block, ConvNeXt
...
class ESANet(nn.Module):
    def __init__(self,
                 ...
                 width=640,
                 num_classes=37,
                 encoder_rgb='convnext-xt',
                 encoder_depth='convnext-xt',
                 encoder_block='Block',
        ...
        # rgb encoder
        if encoder_rgb == 'convnext-xt':
            self.encoder_rgb = ConvNeXt()

        # depth encoder
        if encoder_depth == 'resnet18':
            self.encoder_depth = ConvNeXt()
        else:
            raise NotImplementedError(
